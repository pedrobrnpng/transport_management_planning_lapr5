...
namespace metadataviagens.Tests.integration
{
    public class ServicosViaturaIntegrationTests
    {
        ...
        public void Setup()
        {
            ...
            this._criarServicoViaturaDto = new CriarServicoViaturaDto("Teste", "RGB(10,10,10)", "isDepot", "123123123", listBloc);
            this._servicoViaturaDto = new ServicoViaturaDto(new Guid(), "Teste", "RGB(10,10,10)", "isDepot", "123123123", listBloc);
            var viatura = new Viatura("12-13-GX", new TipoViaturaId("Teste"), "JH4DA9460P2008002", DateTime.Parse("12/12/2010"));
            this._servicoViatura = new ServicoViatura("Teste", new Cor("RGB(10,10,10)"), "isDepot", viatura, this._listBlocos);
            ...
            this._servicoViaturaRepositoryMock.Setup(t => t.GetByDomainIdAsync(It.IsAny<string>())).Returns(Task.FromResult(this._servicoViaturaNull));
            this._servicoViaturaRepositoryMock.Setup(t => t.GetAllAsync()).Returns(Task.FromResult(this._list));
            this._viaturaRepository.Setup(t => t.GetByDomainIdAsync(It.IsAny<string>())).Returns(Task.FromResult(viatura));
            ...
            this._servicosViaturaController = new ServicoViaturaController(this._servicoViaturaService);
        }

        [Test]
        public void ShouldCreateServicoViatura()
        {
            var result = this._servicosViaturaController.Create(this._criarServicoViaturaDto);
            ...
            this._unitOfWorkMock.Verify(u => u.CommitAsync(), Times.AtLeastOnce());
              Assert.IsInstanceOf<Task<ActionResult<ServicoViaturaDto>>>(result);
        }
    }
}