...
//         {
//             var result = this._blocoTrabalhoService.AddAsync(this._criarBlocoTrabalhoDto);
...
//             Assert.AreEqual(this._blocoTrabalhoDto.codigo, result.Result.codigo);
//             Assert.AreEqual(this._blocoTrabalhoDto.horaInicio, result.Result.horaInicio);
//             Assert.AreEqual(this._blocoTrabalhoDto.horaFim, result.Result.horaFim);
//             Assert.AreEqual(this._blocoTrabalhoDto.noInicio, result.Result.noInicio);
//             Assert.AreEqual(this._blocoTrabalhoDto.noFim, result.Result.noFim);
//             Assert.AreEqual(this._blocoTrabalhoDto.ctt, result.Result.ctt);
//             Assert.AreEqual(this._blocoTrabalhoDto.active, result.Result.active);
//         }

//         [Test]
//         public void ShouldGetByDomainId()
//         {
//             var result = this._blocoTrabalhoService.GetByDomainIdAsync("1");

//             this._blocoTrabalhoRepositoryMock.Verify(t => t.GetByDomainIdAsync(It.IsAny<string>()), Times.AtLeastOnce());
//             Assert.AreEqual(this._blocoTrabalhoDto.codigo, result.Result.codigo);
//             Assert.AreEqual(this._blocoTrabalhoDto.horaInicio, result.Result.horaInicio);
//             Assert.AreEqual(this._blocoTrabalhoDto.horaFim, result.Result.horaFim);
//             Assert.AreEqual(this._blocoTrabalhoDto.noInicio, result.Result.noInicio);
