...
namespace metadataviagens.Tests.Services
{
    public class UsersIntegrationTests
    {
        ...
        {
            var result = this._userService.AddAsync(this._criarUserDto);
            ...
            Assert.AreEqual(this._userDto.nome, result.Result.nome);
            Assert.AreEqual(this._userDto.email, result.Result.email);
            Assert.AreEqual(this._userDto.func, result.Result.func);
        }

        [Test]
        public void ShouldGetByDomainId()
        {
            var result = this._userService.GetByDomainIdAsync("1");

            this._userRepositoryMock.Verify(t => t.GetByDomainIdAsync(It.IsAny<string>()), Times.AtLeastOnce());
            Assert.AreEqual(this._userDto.nome, result.Result.nome);
            Assert.AreEqual(this._userDto.email, result.Result.email);
            Assert.AreEqual(this._userDto.func, result.Result.func);
        }
